...
public class ModFiliere extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mod_filiere);
        ...
        btnValider.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ModifierFiliere(btnValider);
            }
        });
        AlertDialog.Builder builderSingle = new AlertDialog.Builder(ModFiliere.this);
        builderSingle.setIcon(R.drawable.ic_check);
        builderSingle.setTitle("Choisir Une Filiere :");

        List<FILIERES> Fill = new Select().from(FILIERES.class).execute();
        final ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(ModFiliere.this, android.R.layout.select_dialog_item);
        for(int i=0;i<Fill.size();i++){
            arrayAdapter.add(Fill.get(i)._CodeFiliere);
        }

        builderSingle.setNegativeButton("Precedent", new DialogInterface.OnClickListener() {
            @Override
        ...
        builderSingle.setAdapter(arrayAdapter, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                F = FILIERES.getByCodeFiliere(strName);
                CODEOrg = F._CodeFiliere;
                idd = F.getId();
                Objects.requireNonNull(txtCodeF.getEditText(),"Matricule EditeText Is NUll").setText(F._CodeFiliere);
                // Log.d(TAG, "onClick: "+Matricule);
                Objects.requireNonNull(txtFNam.getEditText(),"First Name EditeText Is NUll").setText(F._Filiere);
                dialog.dismiss();
            }
        });
        builderSingle.show();
    ...
    private void ModifierFiliere(final MorphingButton btnMorph) {
        if (mMorphCounter == 0) {
            mMorphCounter++;
            ClearFileds();
            ...
            if (CodeInputValidate() & FnameInputValidate() ) {
                ...
                Runnable r = new Runnable() {
                    public void run() {
                        ...
                        try {
                            FILIERES filieres = FILIERES.load(FILIERES.class, idd);//1 is the id
                            filieres._CodeFiliere = CodeF;
                            filieres._Filiere = Fname;
                            filieres.save();
                            mMorphCounter=0;

                            Toast.makeText(getApplicationContext(), "Filiere a ete Modifier", Toast.LENGTH_LONG).show();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
